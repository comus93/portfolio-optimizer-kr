...
def build_optimizer_shared_sections(
    ...
    *,
    ...
    add("#drawdown-chart .chart", not drawdown_series.empty, _drawdown_presentation(drawdown_series, drawdowns, portfolio_order, benchmark_label, drawdown_resilience))
  ...
  const mount = () => {{
    ...
    document.querySelectorAll('.shared-historical-host .chart-mark[data-tooltip]').forEach(mark => {{
      const show = event => {{
        ...
        const tipWidth = tip.offsetWidth || 180; const tipHeight = tip.offsetHeight || 80;
        const pointerX = x - bounds.left + host.scrollLeft;
        const pointerY = y - bounds.top + host.scrollTop;
