...
#loyo-robustness .loyo-matrix thead th { background: #f1f5fb; color: #334155; text-align: center; }
...
(() => {
  ...
  const render = () => {
    ...
    const influenceBody = years.map(year => metrics.map((metric, index) => {
      ...
      return `<tr class="${index === 0 ? 'loyo-year-start ' : ''}${kind === 'delta' ? 'loyo-delta-row' : ''}">`
    ...
    const influenceTable = `
      <div class="loyo-block">
        ...
        <div class="loyo-scroll"><table class="loyo-matrix loyo-influence">
          <thead><tr><th>Year</th><th>Metric</th>${assets.map(asset => `<th>${esc(asset)}</th>`).join('')}</tr></thead>
    ...
    const allocationTable = `
      <div class="loyo-block">
        ...
        <div class="loyo-scroll"><table class="loyo-matrix loyo-allocation">
          <thead><tr><th>Excluded Year</th>${assets.map(asset => `<th>${esc(asset)}</th>`).join('')}</tr></thead>
