...
Backtest report 상단은 Time Period mode, requested/effective period, initial balance, benchmark, portfolio names, run-level rebalancing policy와 Calendar Aligned setting을 확인할 수 있어야 한다.
...
Backtest v1 report는 distribution income breakdown 또는 `Display Income` section을 required/supported section으로 제공하지 않아야 한다. Canonical total return은 유지하되 income decomposition을 임의로 price data에서 추정해서는 안 된다.
