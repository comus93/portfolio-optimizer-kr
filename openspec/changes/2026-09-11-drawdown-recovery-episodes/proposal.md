...
현재 Drawdown report는 episode별 Start, Bottom, Recovery와 화면상 Recovery Time을 제공하지만, 회복 속도는 canonical analytics로 보존되지 않는다. 사용자는 낙폭의 깊이뿐 아니라 바닥 이후 얼마나 빠르게 이전 고점을 회복하는지, 즉 회복탄력성을 별도 축으로 검토하려 한다.

실제 report 검토 결과 `Recovery Progress from Bottom` curve는 현재 의사결정에 유의미한 시사점을 주지 않아 기본 presentation에서 제외한다. canonical recovery progress data는 후속 분석 가능성을 위해 유지한다.
...
- shared Drawdowns presentation은 기존 episode table에 Recovery Rate를 추가한다.
