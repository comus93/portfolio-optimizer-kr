...
Backtest run은 Optimization run과 구분되는 독립 product mode여야 하며 optimization objective나 Efficient Frontier 계산 없이 실행될 수 있어야 한다(MUST).
...
비교 portfolio는 asset row의 union을 공유할 수 있어야 하며 각 portfolio는 각 asset에 독립적인 target weight를 가져야 한다(MUST). 사용하지 않는 asset은 0% weight로 표현할 수 있어야 한다(MUST).
...
각 portfolio의 target weight 합은 100%여야 한다(MUST).
...
Backtest settings는 `Month-to-Month`와 `Year-to-Year` Time Period mode를 선택할 수 있어야 한다(MUST).
...
동일 Backtest run의 모든 portfolio와 benchmark는 동일 requested analysis period를 사용해야 한다(MUST).
...
Backtest run은 positive initial balance를 입력받아 각 portfolio의 wealth/balance path를 동일 시작금액에서 생성할 수 있어야 한다(MUST).
...
Backtest run은 optional benchmark asset을 지정할 수 있어야 한다(MUST).
...
Backtest settings는 `Calendar Aligned = Yes`와 `Calendar Aligned = No`를 모두 지원해야 한다(MUST). Calendar alignment는 periodic rebalancing schedule의 anchor semantics를 결정하며 모든 비교 portfolio에 동일하게 적용되어야 한다(MUST).
...
한 Backtest run의 모든 portfolio는 동일한 periodic rebalancing setting을 사용해야 하며 `none`, `yearly`, `semiannual`, `quarterly`, `monthly` 중 하나를 선택할 수 있어야 한다(MUST). Rebalance bands는 v1에서 제외해야 한다(MUST).
...
같은 asset return matrix와 같은 run-level rebalancing setting을 사용하는 여러 portfolio라도 각 portfolio의 target allocation에 따라 독립적인 weight, return, wealth path를 가져야 한다(MUST).
...
Backtest는 asset return을 별도의 dividend reinvestment option으로 재정의하지 않고 shared `market-data` capability가 제공하는 canonical total-return series를 사용해야 한다(MUST).
...
Backtest product의 primary result는 realized historical analytics여야 하며 optimization-specific expected return, expected covariance frontier, optimized weight 같은 ex-ante 결과를 Backtest 결과로 요구해서는 안 된다(MUST NOT).
...
Backtest에서 기존 shared historical analytics와 동일한 의미를 사용하는 metric은 `portfolio-analytics`의 canonical behavior를 재사용해야 하며 product spec에서 다른 계산 convention을 정의해서는 안 된다(MUST).
...
Backtest v1은 cashflow contribution/withdrawal, rebalance bands, leverage, display income, style analysis, factor regression, regime performance를 canonical input 또는 required calculation behavior로 제공하지 않아야 한다(MUST NOT). Dividend reinvestment는 별도 toggle로 제공하지 않아야 한다(MUST NOT).
...
Portfolio Visualizer snapshot은 feature/information-architecture reference로만 사용해야 하며 PV의 값이나 pixel-level UI 구현 차이 자체를 acceptance failure로 간주해서는 안 된다(MUST NOT).
