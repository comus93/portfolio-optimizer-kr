...
## 25. Research Interpretation Contract

### 25.1 Purpose

LLM은 run 결과를 단순 요약하는 metric narrator가 아니라 **portfolio research analyst**로 동작한다.

분석의 중심 질문은 다음이다.

> 이 experiment의 결과가 연구 질문에 어떤 증거를 제공하며, 기존 portfolio 대비 경제적으로 의미 있는 marginal utility와 robust한 allocation 근거를 제공하는가?

개별 자산의 standalone CAGR이나 Sharpe가 높다는 사실만으로 편입 가치를 판단하지 않는다. 자산 편입 연구에서는 portfolio 전체에 대한 변화와 diversification 효과가 우선이다.

LLM은 다음을 구분해서 다룬다.

```text
Calculated facts      engine이 계산한 값과 관측 가능한 패턴
Interpretation        facts에 대한 금융공학/portfolio 관점의 해석
Conclusion            현재 연구 질문에 대한 판단
Follow-up             남은 불확실성을 줄이기 위한 다음 experiment
```

해석은 연구 질문에 필요한 항목에 집중한다. 모든 metric을 기계적으로 나열하는 checklist report를 만들지 않는다.

### 25.2 Default LLM Read Path

GitHub read 횟수와 불필요한 raw-data 탐색을 줄이기 위해 research run은 다음 순서로 읽는다.

```text
1. studies/<study-id>/study.md
   - 연구 질문, 과거 결론, 현재 follow-up 확인

2. runs/<run_id>/context.yaml
   - exact study / experiment linkage 확인

3. runs/<run_id>/review/research_summary.json
   - 이번 run의 compact research packet 확인

4. 필요한 경우에만 상세 artifact drill-down
   - efficient_frontier.csv
   - correlations.csv
   - drawdowns.csv
   - rolling_returns_3y.csv / rolling_returns_5y.csv
   - monthly_return_series.csv
   - active_returns.csv
   - raw/* 또는 result.json
```

`result.json`은 canonical source of truth이며 언제든 상세 검증에 사용할 수 있다. `research_summary.json`은 canonical result를 대체하지 않고 LLM이 첫 분석을 시작하기 위한 compact derived view다.

### 25.3 Research Summary Packet

Research run은 다음 파일을 제공한다.

```text
runs/<run_id>/review/research_summary.json
```

이 파일은 새로운 금융 metric을 계산하지 않는다. 기존 canonical result와 review table의 값을 선택·정리하고 상세 artifact 위치를 알려주는 역할만 한다.

최소 포함 내용:

```text
run_id
objective / target volatility when applicable
analysis period and actual data coverage
benchmark overlap when applicable
provided / optimized allocation and min/max bounds
provided / optimized / benchmark core performance metrics
optimized expected return / volatility / ex-ante Sharpe
benchmark active-return / tracking-error / information-ratio summary
risk contribution summary
return contribution summary
rolling-return summary
major drawdown summary
artifact index for detailed tables
```

`artifact index`는 최소한 다음 상세 review artifact를 가리킬 수 있어야 한다.
...
해당 기능이 적용되지 않는 run에서는 관련 field나 artifact가 없을 수 있으며 이를 명시적으로 표현한다.

### 25.4 Data Adequacy and Statistical Context

LLM은 금융 해석 전에 데이터가 해당 결론을 지지할 수 있는지 먼저 확인한다.

주요 확인 대상:

- requested period와 actual common coverage의 차이
- 관측 월 수와 track record 길이
- 최근 상장 자산 때문에 전체 universe 기간이 짧아졌는지
- mixed-currency 환산 여부와 base currency
- benchmark comparison의 실제 overlap period
- 특정 짧은 시기나 이상치가 결과를 과도하게 지배하는지

상장 이력이 짧은 자산 때문에 10년 연구가 2~3년 표본으로 줄었다면 optimizer가 수치상 답을 생성했더라도 장기 portfolio 결론의 evidence strength는 낮게 평가한다.

Data coverage 문제가 핵심이면 이를 optimizer 결과보다 먼저 report한다.

### 25.5 Financial Engineering Analysis

LLM은 다음 관점에 우선적으로 주목한다.

#### Objective와 최적점의 의미

- optimized portfolio가 사용한 objective가 무엇인지 확인한다.
- Target Volatility에서는 target constraint가 실제로 binding하는지 확인한다.
- Maximum Sharpe에서는 특정 asset의 min/max constraint가 결과를 결정하고 있는지 확인한다.

Asset weight가 `max_weight`에 붙었다면 그 숫자를 "최적 비중"으로 해석하지 않는다. 이는 **optimizer가 허용 범위 바깥에서도 더 높은 비중을 원할 가능성이 있음을 보여주는 active constraint**다.

Asset weight가 0에 가깝다는 사실 역시 현재 sample, expected-return estimate, covariance와 constraints 아래에서 marginal allocation value가 낮다는 의미로 해석하며 자산 자체가 무가치하다는 결론으로 확대하지 않는다.

#### Efficient Frontier와 allocation stability

단일 optimized point보다 Efficient Frontier 전체를 중요하게 본다.

확인할 내용:

- frontier를 따라 risk/return trade-off가 어떻게 변하는가
- candidate asset weight가 넓은 frontier 구간에서 유지되는가
- 특정 좁은 corner에서만 급격히 등장하는가
- 작은 risk 변화에 allocation이 급변하는가
- 비슷한 expected return / volatility를 주는 넓은 allocation plateau가 존재하는가

Portfolio research의 실전 결론은 가능한 경우 정확한 `27.43%`보다 **안정적인 allocation range**로 표현한다.

#### Expected와 Realized의 구분

다음을 구분한다.

```text
Expected Return / optimization volatility / ex-ante Sharpe
    = optimization model의 sample statistics

CAGR / realized volatility / drawdown / ex-post Sharpe
    = historical path에서 실제로 발생한 결과
```

Expected Return을 미래 수익률 forecast처럼 표현하지 않는다.

Ex-ante와 ex-post 결과가 크게 다르면 arithmetic expected return, volatility, compounding/path dependency, rebalancing 효과 등을 가능한 원인으로 검토한다.

#### Correlation, Covariance, Diversification

낮은 correlation 자체를 최종 결론으로 삼지 않는다.

다음을 함께 본다.

- candidate와 기존 주요 자산의 correlation
- portfolio volatility 변화
- risk contribution 변화
- drawdown 및 rolling performance 변화
- 해당 correlation이 연구 기간 전체에 의존한 결과인지

Diversification thesis가 correlation 안정성에 크게 의존하지만 rolling/sub-period correlation data가 없다면 안정성을 추정해서 말하지 않고 이를 follow-up 분석 대상으로 남긴다.

### 25.6 Portfolio Management Analysis

Portfolio 관점에서는 standalone asset ranking보다 **baseline 대비 portfolio delta**를 우선한다.

자산 편입 또는 weight 변경 연구에서는 가능한 경우 다음을 비교한다.

```text
Current / Provided Portfolio
        vs
Optimized or Candidate-added Portfolio
```

주요 질문:

- expected/realized return은 얼마나 개선되거나 악화됐는가
- volatility와 MDD는 어떻게 변했는가
- Sharpe / Sortino는 개선됐는가
- risk contribution이 capital weight에 비해 과도하게 집중되는가
- return contribution과 risk contribution의 균형은 어떤가
- benchmark 대비 active return 개선이 tracking error 증가를 정당화하는가
- 기존 자산과 중복되는 exposure인지, 독립적인 portfolio role을 제공하는가

Candidate가 높은 standalone return을 보이더라도 기존 portfolio와 높은 상관성 때문에 portfolio-level risk-adjusted result가 개선되지 않으면 편입 근거는 약하다.

반대로 standalone return이 낮아도 volatility, drawdown, correlation 구조를 통해 전체 portfolio의 efficient frontier를 개선한다면 의미 있는 diversifier가 될 수 있다.

### 25.7 Temporal Robustness and Data Analysis

Full-period aggregate만 보고 결론을 내리지 않는다.
...
- drawdown depth와 recovery duration
- benchmark active-return path
- 필요 시 monthly return series의 stress period

주요 질문:

- 결과가 여러 구간에서 같은 방향을 유지하는가
- 특정 bull/bear period 하나가 전체 결론을 지배하는가
- optimized portfolio의 개선이 평균값뿐 아니라 bad period에서도 의미가 있는가
- rolling 결과의 dispersion이 너무 커서 full-period 평균이 대표성을 잃는가

단일 run에서 확인 가능한 robustness와 별도 experiment가 필요한 robustness를 구분한다.

```text
Within-run
- frontier allocation transition
- full / trailing / rolling performance
...
- 다른 analysis period
- 다른 objective
- 다른 min/max constraint
- asset universe 변경
- 특정 baseline 제거/추가 비교
```

### 25.8 Interpretation Guardrails

LLM은 다음 오류를 피한다.

- 한 개 optimized weight를 정밀한 미래 최적 비중처럼 제시하지 않는다.
- max/min bound에 붙은 weight를 unconstrained optimum으로 해석하지 않는다.
- Expected Return을 forecast와 동일시하지 않는다.
- standalone CAGR 하나로 portfolio 편입을 권하지 않는다.
- full-period low correlation만으로 안정적인 diversification을 단정하지 않는다.
- PV parity 차이를 portfolio investment insight로 해석하지 않는다.
- 계산되지 않은 metric이나 확인하지 않은 regime behavior를 만들어내지 않는다.
- 계산된 사실과 LLM의 해석/가설을 섞지 않는다.

현재 output으로 답할 수 없는 중요한 질문이 발견되면 억지로 결론을 만들기보다 어떤 데이터 또는 follow-up experiment가 필요한지 명시한다.

반복적으로 필요한 분석 데이터가 현재 canonical/review output에 없다면 일회성 LLM 계산을 관행화하지 않고 engine output contract와 test에 추가하는 방향을 검토한다.

### 25.9 Research Conclusion

각 run 분석은 `study.md`에 다음 수준의 결론을 남길 수 있어야 한다.

```text
Observed facts
- 연구 질문에 직접 관련된 핵심 계산 결과

Interpretation
- 왜 그 결과가 발생했는지에 대한 금융공학 / portfolio 관점의 해석

Current conclusion
- 연구 질문에 대한 현재 답
- 방어 가능한 경우 stable allocation range 또는 portfolio role
- evidence strength를 낮추는 주요 caveat

Follow-up
- 결론을 바꿀 가능성이 있는 남은 불확실성
- 그 불확실성을 직접 검증하는 다음 experiment
```

Follow-up은 단순히 더 많은 parameter를 돌리기 위한 것이 아니라 **현재 결론에서 가장 중요한 불확실성을 줄이기 위한 실험**이어야 한다.

### 25.10 Research Interpretation Acceptance Checks

Research Interaction 구현에서는 최소한 다음을 검증한다.

1. research run에 `review/research_summary.json`이 생성된다.
2. `research_summary.json`의 값은 `result.json` 또는 기존 review/raw artifact에서 추적 가능하며 새로운 금융 계산을 source of truth로 만들지 않는다.
3. summary에는 run identity, objective, data coverage, allocation, core performance가 포함된다.
4. benchmark가 있으면 benchmark overlap과 active-risk summary를 포함한다.
5. risk/return decomposition이 존재하면 summary에서 접근할 수 있다.
6. rolling/drawdown 결과가 존재하면 summary에서 접근할 수 있다.
7. detailed artifact index가 실제 존재하는 review file을 가리킨다.
8. 적용되지 않는 optional analysis 영역은 누락을 숨기지 않고 `null`, 빈 값 또는 명시적 unavailable 상태로 표현한다.
9. 기존 `result.json`이 canonical source of truth라는 계약은 유지된다.
10. research summary 추가 후 기존 전체 regression suite가 통과한다.
